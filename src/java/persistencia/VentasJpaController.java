...
public class VentasJpaController implements Serializable {
    ...
    public void create(Ventas ventas) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            em.persist(ventas);
            if (cliente != null) {
                cliente.getListCompras().add(ventas);
    ...
    public void edit(Ventas ventas) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Cliente clienteOld = persistentVentas.getCliente();
            Cliente clienteNew = ventas.getCliente();
            if (clienteNew != null) {
            ...
            if (clienteOld != null && !clienteOld.equals(clienteNew)) {
                clienteOld.getListCompras().remove(ventas);
                clienteOld = em.merge(clienteOld);
            }
            if (clienteNew != null && !clienteNew.equals(clienteOld)) {
                clienteNew.getListCompras().add(ventas);
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            Cliente cliente = ventas.getCliente();
            if (cliente != null) {
                cliente.getListCompras().remove(ventas);
                cliente = em.merge(cliente);
            }
            em.remove(ventas);
            em.getTransaction().commit();
